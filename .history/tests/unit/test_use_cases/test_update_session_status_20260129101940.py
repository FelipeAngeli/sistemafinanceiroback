...
class TestUpdateSessionStatusUseCase:
    ...
    async def test_realizar_sessao_cria_lancamento_financeiro(
        ...
        input_data = UpdateSessionStatusInput(
            user_id=agendada_session.user_id,
        ...
        pending = await financial_repo.list_pending(user_id=agendada_session.user_id)
    ...
    async def test_realizar_sessao_com_notas(
        ...
        # Assert
        updated = await session_repo.get_by_id(user_id=agendada_session.user_id, session_id=agendada_session.id)
        assert updated is not None, "Sessão deve existir após atualização"
